...
function rememberTaskContext(jobId) {
  const task = state.taskStore.get(jobId) || {};
  state.taskStore.set(jobId, {
    ...task,
    id: jobId,
    filename: state.selectedFile?.name || "未命名需求图",
...
function updateSubmitState() {
  ...
  elements.submitButton.disabled = !state.ready || !state.selectedFile || busy;
  elements.configTrigger.disabled = state.submitting || state.runningJobIds.size > 0;
}

async function submitJob(event) {
  event.preventDefault();
  if (!state.selectedFile || !state.ready || state.submitting) return;
  ...
  elements.intakeSummary.textContent = `${state.selectedFile.name} · 已锁定为当前图1`;
  updateSubmitState();
  setInputLocked(true);
  prepareResultForProcessing();
  showProcess("queued", "正在提交", "正在安全上传需求图。");
