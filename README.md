...
在页面“服务配置”中填写 PixPark Token 并打开“允许确认提示词后生成结果图”。提示词通过审查后不会自动进入 PixPark；用户需要在正向提示词页签中检查内容，再点击“确认提示词，双模型生成 4 张”。也可使用 `.env`：
...
- `MAX_CONCURRENT_IMAGE_JOBS=0` 表示 PixPark 图片任务并发不设本地上限，也是默认值；设置正整数可重新启用上限。每个图片任务内部的 Nano 1 路与 GPT 2 路也会并发创建、轮询；图片轮询不会占用模型解析并发。
